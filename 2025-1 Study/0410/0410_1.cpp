...
// int space_map[MAXN][MAXN];      // 실제 미지 공간 평면도
// int space_map_node[MAXN][MAXN]; // 미지의 공간 평면도의 그래프 노드 저장
// int time_wall[6][MAXM][MAXM];   // 실제 시간의 벽
// int time_wall_node[6][MAXM][MAXM]; // 시간의 벽의 그래프 노드 저장
...
// vector<vector<int> > graph; // 그래프 저장할 인접리스트
// int dist[2400]; // ?? -> 타임머신에서 해당 번호의 셀까지 도달하는 데 필요한 최소 턴 횟수라네요
// int N, M, F;

// void build_graph(); // 그래프 설계
...
//     // BFS
//     fill(dist, dist + (N * N) + 4 * (M * M) + 1, -1); // 거리 함수 초기화
//     // 도달 불가능한 경우 0 출력 위해 -1로 초기화

//     for(int i = 0; i < N; i++) {
//         for(int j = 0; j < N; j++) {
//             if(space_map[i][j] == 3) continue;
//             if(space_map[i][j] == 1) {
//                 int idx = space_map_node[i][j];
//                 dist[idx] = INT_MAX;
//             }
//         }
//     }
...
//         for(int i = 0; i < N; i++) {
//             for(int j = 0; j < N; j++) {
//                 if(time_wall[m][i][j] == 1) {
//                     int idx = time_wall_node[m][i][j];
//                     dist[idx] = INT_MAX;
//                 }
//             }
//         }
...
//                 cell_start = time_wall_node[4][i][j];
//                 break;
//             }
//         }
//     }
...
//     for(int runs = 1;;runs++) {
//         // 현재 턴에 맞는 이상현상 존재 시 영향 받는 셀 업데이트
//         for(int i = 0; i < F; i++) {
//             if(!events[i].Alive) continue;
//             if(runs % events[i].V) continue;

//             int nr = events[i].R, nc = events[i].C;
            
//             if(events[i].Dir == 0) nc += (runs / events[i].V);
//             else if(events[i].Dir == 1) nr += (runs / events[i].V);
//             else if(events[i].Dir == 2) nc -= (runs / events[i].V);
//             else nr -= (runs / events[i].V);
...
//             int idx = space_map_node[nr][nc];
//             dist[idx] = INT_MAX;
//         }

//         vector<int> next_cells; // 현재 턴에 도달 가능한 셀 위치 저장하는 vector
...
//                 if(next == -1) continue; // 이동가능한 셀이 없음
//                 if(dist[next] != -1) continue; // 이미 방문한 적 있음

//                 dist[next] = runs;
//                 next_cells.push_back(next); // 새로 도달 가능한 셀의 번호 추가
//             }
//         }

//         if(!(int)next_cells.size()) break; // 도달 가능한 셀이 없으면 종료
...
//         // 탈출구에 도달할 경우 종료
//         if(dist[cell_end] != -1) break;
//     }

//     cout << dist[cell_end] << endl;
//     return 0;
// }
...
//     for(int i = 0; i < N; i++)
//         for(int j = 0; j < N; j++)
//             if(space_map[i][j] != 3) // 시간의 벽이 위치한 곳이 아니면 번호 부여
//                 space_map_node[i][j] = ++cnt;
    
//     for(int m = 0; m < 5; m++)
//         for(int i = 0; i < M; i++)
//             for(int j = 0; j < M; j++)
//                 time_wall_node[m][i][j] = ++cnt;

//     graph = vector<vector<int> >(cnt + 1, vector<int>(4, -1)); // 그래프 초기화

//     int dy[4] = { 1, 0, -1, 0 }; // 동 남 서 북 순서
//     int dx[4] = { 0, 1, 0, -1 };
...
//                     if(space_map[nr][nc] == 3) continue;

//                     graph[idx][d] = space_map_node[nr][nc];
//                 }
//             }
//         }
//     }
...
//                     if(nc < 0) graph[idx][d] = time_wall_node[(m + 1) % 4][nr][M - 1];

//                     else if(nc >= M) graph[idx][d] = time_wall_node[(m + 3) % 4][nr][0];

//                     else graph[idx][d] = time_wall_node[m][nr][nc];
//                 }
//             }
//         }
//     }
...
//                 graph[idx][d] = time_wall_node[4][nr][nc];
//             }
//         }
//     }
//     // 무조건 옆면의 셀은 북쪽으로 연결됨
//     // 동쪽 단면도와 인접한 셀
//     for(int i = 0; i < M; i++) {
//         int top = time_wall_node[4][i][M - 1];      // 윗면의 가장 오른쪽에 있는 노드들
//         int east = time_wall_node[0][0][M - i - 1]; // 동쪽의 가장 윗쪽 노드와 연결
//         graph[top][0]= east;
//         graph[east][3] = top;
//     }
//     // 남
//     for(int i = 0; i < M; i++) {
//         int top = time_wall_node[4][M - 1][i];
//         int south = time_wall_node[1][0][i];
//         graph[top][1] = south;
//         graph[south][3] = top;
//     }
//     // 서
//     for(int i = 0; i < M; i++) {
//         int top = time_wall_node[4][0][M - 1];
//         int west = time_wall_node[2][0][i];
//         graph[top][2] = west;
//         graph[west][3] = top;
//     }
//     // 북
//     for(int i = 0; i < M; i++) {
//         int top = time_wall_node[4][0][i];
//         int north = time_wall_node[3][0][M - i - 1];
//         graph[top][3] = north;
//         graph[north][3] = top;
//     }
    
//     int time_wall_start_r = -1, time_wall_start_c = -1;

//     // 평면도에서 시간의 벽이 시작하는 행 찾기
//     for(int i = 0; i < N; i++) {
//         for(int j = 0; j < N; j++) {
//             if(space_map[i][j] == 3) {
//                 time_wall_start_r = i;
...
//             }
//         }
//         if(time_wall_start_r != -1) break;
//     }

//     // 평면도와 시간의 벽 사이 연결
//     // 동
//     if(time_wall_start_c + M < N) {
//         for(int i = 0; i < M; i++) {
//             int time = time_wall_node[0][M - 1][i];
//             int map = space_map_node[time_wall_start_r + (M - i - 1)][time_wall_start_c + M];
//             graph[time][1] = map;
//             graph[map][2] = time;
//         }
//     }
//     // 남
//     if(time_wall_start_r + M < N) {
//         for(int i = 0; i < M; i++) {
//             int time = time_wall_node[1][M - 1][i];
//             int map = space_map_node[time_wall_start_r + M][time_wall_start_c + i];
//             graph[time][1] = map;
//             graph[map][3] = time;
//         }
//     }
//     // 서
//     if(time_wall_start_c > 0) {
//         for(int i = 0; i < M; i++) {
//             int time = time_wall_node[2][M - 1][i];
//             int map = space_map_node[time_wall_start_r + i][time_wall_start_c - 1];
//             graph[time][1] = map;
//             graph[map][0] = time;
//         }
//     }
//     // 북
//     if(time_wall_start_r > 0) {
//         for(int i = 0; i < M; i++) {
//             int time = time_wall_node[3][M - 1][i];
//             int map = space_map_node[time_wall_start_r - 1][time_wall_start_c + (M - i - 1)];
//             graph[time][1] = map;
//             graph[map][1] = time;
//         }
//     }
